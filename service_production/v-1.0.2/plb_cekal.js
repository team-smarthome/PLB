...
async function DeleteUserCekal() {
    try {
        const deleteRequests = ipCameraData.map(async (ip) => {
            ...
            const data = UserCekal.filter(item => item?.skor_kemiripan === 100).map(personId => ({
                identityType: 1,
                deletePersonId: 1,
...
async function DeleteUserCekalToKamera(params) {
    try {
        const deleteRequests = ipCameraData.map(async (ip) => {
            const url = `http://${ip?.ipCamera}/facial_api/aiot_call/service/sendPlatformInfo`;

            const data = params.map(personId => ({
                identityType: 1,
                deletePersonId: 1,
                personId: personId?.no_passport,
                reserve: ""
        ...
        console.log("PROSES DELETE DATA SELESAI");
    } catch (error) {
        console.error("Error:", error);
    }
}
...
async function updateDataCekalToApi() {
    console.log("Updating data to API...");
    const dataUserCekal = UserCekal.map(personId => ({
        id: personId?.id,
        skor_kemiripan: personId?.skor_kemiripan,
    }));
...
async function startCheckCekal() {
    try {
        ...
        for (const record of records) {
            ...
            if (responseCekal?.response_code === "25") {
                ...
                if (responseCekal?.data.length > 0) {
                    const isDuplicate = UserCekal.some(item =>
                        item.id === record?.id &&
                        item.no_passport === record?.no_passport &&
                        item.skor_kemiripan === record?.skor_kemiripan
                    );
                    ...
                    if (!isDuplicate) {
                        UserCekal.push({
                            ...
                            skor_kemiripan: responseCekal?.data[0]?.skor_kemiripan,
                        });
                    }
                }
                continue;
        ...
        console.log("UserCekal:", UserCekal.length);

        if (UserCekal.length > 0) {
            await updateDataCekalToApi();
        }

    } catch (error) {
        console.error("Error cek cekal")
    }
...
async function deleteDataFromKamera() {
    ...
    if (delteData?.status === 200) {
        ...
        console.error("Gagal MendapatakaDatayang sudah dihapus");
    }
}
...
async function main() {
    ...
    try {
        ...
        console.log(ipCameraData.length, "TOTAL KAMERA")

        await deleteDataFromKamera();
...
main().catch(console.log("Try Again"));
