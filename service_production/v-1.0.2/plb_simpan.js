...
const handleSimpanPerlintasan = async (data, socket, action) => {
    ...
    try {
        if (action === "log-register") {
            ...
            const { data: logFaceReg } = await axios.get(`http://127.0.0.1/PLB-API/public/api/face-reg-simpan-pelintas`, {
                params: {
                    startDate,
                    endDate,
                    nationality,
                ...
                if (apiSimpanPerlintasan.data.response_code == '00') {
                    ...
                    try {
                        const apiUpdateDataUser = await axios(
                            {
                                method: "patch",
                                url: `http://127.0.0.1/PLB-API/public/api/update-sync-status-fr`,
                                data: {
...
async function CheckCekal(data, params) {
    ...
    try {
        const { data: checkCekal } = await axios.post('http://127.0.0.1/PLB-API/public/api/cek-cekal', DataCekCekal);
