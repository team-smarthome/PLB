...
const JobTitle = () => {
  ...
  const handleEditPetugas = async () => {
    try {
      setIsLoading(true);
      const res = await UpdateJabatan(formData.id, {
  ...
  const customRowRenderer = (row) => {
    return (
      <>
        <td className="text-center">{row?.nama_jabatan}</td>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Kelola data jabatan yang terdaftar dalam sistem.
          </p>
        </div>
          ...
          <div className="flex items-center gap-4 w-full mt-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                onChange={(e) =>
                  setSearch({ ...search, nama_jabatan: e.target.value })
                }
              />
            </div>
            <button
